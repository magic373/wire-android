...
class SSOWebViewFragment extends FragmentHelper {
  ...
  override def onViewCreated(view: View, savedInstanceState: Bundle): Unit = {
    ...
    webView.foreach { webView =>
      ...
      for {
        result <- webViewWrapper.loginWithCode(getArguments.getString(SSOCode))
        _      <- ssoResponse(result)
      } yield ()
    }
  ...
  private def ssoResponse(loginResult: SSOResponse): Future[Unit] = loginResult match {
