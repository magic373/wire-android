...
class SingleParticipantFragment extends FragmentHelper {
  ...
  private lazy val tabs = returning(view[TabLayout](R.id.details_and_devics_tabs)) {
    _.foreach {
  ...
  private lazy val userHandle = returning(view[TextView](R.id.user_handle)) { vh =>
    ...
    handle.map(_.isDefined).onUi(vis => vh.foreach(_.setVisible(vis)))
    handle.map {
  ...
  private lazy val footerCallback = new FooterMenuCallback {
    ...
    override def onRightActionClicked(): Unit =
      (for {
        ...
        convId  <- inject[ConversationController].currentConvId.head
      } yield (isGroup, convId)).flatMap {
        case (false, convId) => Future.successful(inject[IConversationScreenController].showConversationMenu(false, convId))
  ...
  override def onViewCreated(v: View, @Nullable savedInstanceState: Bundle): Unit = {
    ...
    detailsView.foreach { view =>
      ...
      (for {
        ...
        rrTitle         =  if (rrVisible) Some(getString(
                             if (receiptsEnabled) R.string.read_receipts_info_title_enabled
